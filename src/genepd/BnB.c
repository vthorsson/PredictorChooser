...
stack *BranchBound (stack *inequalSet, int num_vars) {
  ...
  /* try BnB with min_cost of 2, then increase if no result returned */
  min_cost = 2;
  build_tree (root, inequalSet, inequalSet->top, result, 
	      &curr_soln, &min_cost);
  while (empty_stack(result) == TRUE) {
    min_cost++;
    fprintf(stderr, "\n****Branch and Bound returned no result****\n");
    fprintf(stderr, "Trying again w/ min_cost = %d\n\n", min_cost);
    build_tree (root, inequalSet, inequalSet->top, result, 
		&curr_soln, &min_cost);
  }
...
void build_tree (tree_node *curr_tree, stack *inequalSet, 
  ...
  if (satisfyTest(inequalSet, *curr_soln) == TRUE) {
    ...
    else printf("Already in stack\n");

  } else if ( curr_cost >= *min_cost_ptr) {
    ...
    /* if (++levelNumber % 10 == 0) printf("Level #: %d\n", levelNumber); */
