...
/* retrieves a particular level w/o popping or pushing the stack  */
/* returns TRUE if the stack had valid data at the specified # of */
/* levels deep.                                                   */
...
BOOL get_entry (stack *aStack, int level, BitVectorType *bv) {
  ...
  CopyBitVector(stack_ptr->data, bv);
  return TRUE;
}
